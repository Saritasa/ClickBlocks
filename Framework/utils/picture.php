...
class Picture
{
  ...
  public function crop($left, $top, $width, $height, $bgcolor = 0, $fixedSize = true)
  {
    ...
    if ($right < 0 || $bottom < 0 || $left >= $w || $top >= $h)
    {
      ...
      return $this->img = $this->createNewImage($width, $height, $bgcolor);
    }
    $x = $left; $y = $top;
    if ($x < 0) $x = 0;
    if ($y < 0) $y = 0;
  ...
  public function resize($width, $height, $mode = self::PIC_AUTO, $maxWidth = null, $maxHeight = null)
  {
    ...
    if (false !== $res = imagecopyresampled($new, $img, 0, 0, 0, 0, $width, $height, $w, $h))
    {
      $this->info['width'] = $width;
      $this->info['height'] = $height;
      return $this->img = $new;
  ...
  protected function readImageInfo($image)
  {
    ...
    $this->info['bits'] = isset($info['bits']) ? $info['bits'] : 0;

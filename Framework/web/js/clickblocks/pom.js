...
var POM = (function(undefined)
{
  ...
  POM.prototype._refreshPOMTree = function(data)
  {
    ...
    for (id in params)
    {
      ...
      if ((ctrl instanceof Panel) && (typeof params[id] != 'object'))
      {
        for (i in panels)
        {
    ...
    for (id in params)
    {
      ...
      {
        ctrl.refresh(params[id]['attrs'], params[id]['removed']);
        for (var i in params[id]['removed']) {
          delete vs[id].attrs[params[id]['removed'][i]];
        }
        $.extend(vs[id].attrs, params[id]['attrs']);
      }
      else
      {
        ctrl.refresh(params[id]);
        vs[id] = ctrl.vs();
      }
    }
    return this;

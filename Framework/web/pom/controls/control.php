...
abstract class Control
{
  ...
  /**
   * Returns delegate string for the given control method, with slashes escaped (suitable to put directly in JS code).
   *
  ...
  {
    return addcslashes($this->callbackString($callback, $isStatic), '\\');
  }
     ...
     * @access public
     */
    public function callbackString($callback = '', $isStatic = false)
    {
        return get_class($this) . ($isStatic ? '::' . $callback : '@' . $this->attributes['id'] . '->' . $callback);
    }
   ...
   *
   * @return ClickBlocks\Web\POM\Panel
   * @access public
   */
   ...
   *
   * @param ClickBlocks\Web\POM\Panel $parent - the control parent.

...
namespace
{
     ...
     * @return string
     * @throws \ClickBlocks\Core\Exception
     */
  function ID($id)
  {
...
class View implements \ArrayAccess
{
  ...
  /**
   * An instance of ClickBlocks\Core\Template class that contains the page HTML.
   *
  ...
  {
    $value = \CB::exe(static::decodePHPTags($value, $marks ?: []), ['config' => \CB::getInstance()->getConfig()]);
    if (substr($value, 0, strlen(self::PHP_MARK)) == self::PHP_MARK)
    {
      $value = substr($value, strlen(self::PHP_MARK));
      if (strlen($value) == 0) return;
      eval(\CB::ecode('$value = ' . htmlspecialchars_decode($value) . ';'));
    }
    return $value;
  }
  ...
  public function addJS(array $attributes, $script = null, $inHead = true, $order = null)
  {
    ...
    $id = isset($attributes['src']) ? $attributes['src'] : count($this->js[$place]);;
    return $this->setJS($id, $attributes, $script, $inHead, $order);
  }
  ...
  public function setJS($id, array $attributes, $script = null, $inHead = true, $order = null)
  {
    ...
    $newItem = [
        'script' => $script,
        'attributes' => $attributes,
        'order' => $order !== null ? (int)$order : count($this->js[$place])
    ];
    if (isset($this->js[$place][$id])) $newItem['order'] = $this->js[$place][$id]['order'];
    $this->js[$place][$id] = $newItem;
    return $this;
  }
  ...
  {
    $args = func_get_args();
    $config = \CB::getInstance()->getConfig();
   ...
   *
   * @param \ClickBlocks\Web\POM\Control $ctrl - a control to be added to the view.
   * @return self
   * @access public
   */
   ...
   *
   * @param string|\ClickBlocks\Web\POM\Control $ctrl - a control object or its unique identifier.
   * @return boolean
   * @access public
   */
   ...
   * @param \ClickBlocks\Web\POM\Control $context - the panel, only inside which the search should be performed.
   * @return \ClickBlocks\Web\POM\Control
   * @access public
   */
  ...
  public function check($id, $flag = true, $searchRecursively = true)
  {
    ...
    else if ($ctrl instanceof Panel)
    {
      foreach ($ctrl->getControls() as $child)
      {
        ...
        if ($vs['class'] == 'ClickBlocks\Web\POM\CheckBox')
        {
  ...
  public function process($data)
  {
    ...
    $response->setContentType('json', isset(\CB::getInstance()['pom']['charset']) ? \CB::getInstance()['pom']['charset'] : 'utf-8');
